...
export class HighchartsVisualizer implements IVisualizer {
  ...
  public drawNewChart(options: IVisualizerOptions): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.currentChart = ChartFactory.create(
  ...
  public updateExistingChart(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const isChartTypeTheOnlyChange =
          changes.count === 1 && changes.isPendingChange(ChartChange.ChartType);

        if (isChartExist && isChartTypeTheOnlyChange) {
          const oldChart = this.currentChart;
          const newChart = ChartFactory.create(
          ...
          if (
            oldChart.getSplitByCategoriesAndSeries ===
              newChart.getSplitByCategoriesAndSeries &&
            oldChart.getStandardCategoriesAndSeries ===
              newChart.getStandardCategoriesAndSeries
          ) {
            this.currentChart = newChart;
            this.options = options;

            // Build the options that need to be updated
            let newOptions: Highcharts.Options =
              this.currentChart.getChartTypeOptions();

            // Apply the changes
            this.highchartsChart.update(newOptions);

            // Save the new options
            this.basicHighchartsOptions = _.merge(
              {},
              this.basicHighchartsOptions,
  ...
  private draw(
    ...
    try {
      ...
      const highchartsOptions = _.merge(
        {},
        this.basicHighchartsOptions,
        this.themeOptions
      );
      ...
      if (
        updateCustomOptionsFn &&
        typeof updateCustomOptionsFn === "function"
      ) {
      ...
      // Draw the chart
      this.highchartsChart = Highcharts.chart(
        elementId,
        highchartsOptions,
        () => {
          this.handleResize();
      ...
      reject(
        new VisualizerError(ex.message, ErrorCode.FailedToCreateVisualization)
      );
    }
  }

  private onFinishDrawingChart(
    resolve: ResolveFn,
    options: IVisualizerOptions
  ): void {
    ...
    const finishChartAnimationCallback =
      options.chartOptions.onFinishChartAnimation;
  ...
  private getHighchartsOptions(
    options: IVisualizerOptions
  ): Highcharts.Options {
    const chartOptions = options.chartOptions;
    const isDatetimeAxis = Utilities.isDate(
      chartOptions.columnsSelection.xAxis.type
    );
    let animation;
  ...
  private getXAxis(
    ...
    return {
      ...
      labels: {
        formatter: Formatter.getLabelsFormatter(
          chartOptions,
          chartOptions.columnsSelection.xAxis,
          useHTML
        ),
        useHTML: useHTML,
        style: {
  ...
  private getYAxis(
    chartOptions: IChartOptions,
    categoriesAndSeries: Highcharts.Options
  ...
  private getMultipleYAxis(
    ...
    return categoriesAndSeries.series.map((serie, index) => {
      const yAxisOptions: Highcharts.YAxisOptions = {
        ...
        opposite:
          chartOptions.columnsSelection?.splitBy?.[0]?.getPosition?.(
            serie.name
          ) ?? false,
      };
      if (chartOptions.yMinimumValue != null) {
  ...
  private getCategoriesAndSeries(
    options: IVisualizerOptions
  ): Highcharts.Options {
    ...
    if (columnsSelection.splitBy && columnsSelection.splitBy.length > 0) {
      categoriesAndSeries =
        this.currentChart.getSplitByCategoriesAndSeries(options);
    } else {
      categoriesAndSeries =
        this.currentChart.getStandardCategoriesAndSeries(options);
    }

    if (
      !options.chartOptions.columnsSelection.splitBy.some(
        (split) => split.getPosition === undefined
      )
    ) {
      categoriesAndSeries = {
        ...categoriesAndSeries,
  ...
  private onFinishDataTransformation(
    options: IVisualizerOptions,
    resolve: ResolveFn,
    reject: RejectFn
  ): void {
    ...
    if (options.chartOptions.onFinishDataTransformation) {
      const drawChartPromise = options.chartOptions.onFinishDataTransformation(
  ...
  private verifyInput(options: IVisualizerOptions): void {
    ...
    if (!elementId) {
      throw new InvalidInputError(
        "The elementId option can't be empty",
        ErrorCode.InvalidChartContainerElementId
      );
    }
    ...
    if (!document.querySelector(`#${elementId}`)) {
      throw new InvalidInputError(
        `Element with the id '${elementId}' doesn't exist on the DOM`,
        ErrorCode.InvalidChartContainerElementId
      );
    }
    ...
    if (
      columnSelection.yAxes.length > 1 &&
      columnSelection.splitBy &&
      columnSelection.splitBy.length > 0
    ) {
      throw new InvalidInputError(
        "When there are multiple y-axis columns, split-by column isn't allowed",
        ErrorCode.InvalidColumnsSelection
      );
    }
  }
  ...
  private getLegendOptions(
    ...
    if (chartOptions.legendOptions.position === LegendPosition.Bottom) {
      ...
      Highcharts.wrap(
        Highcharts.AccessibilityComponent.prototype,
        "updateProxyButtonPosition",
  ...
  private getLegendMaxHeight(): number {
    ...
    const chartContainer: Element = document.querySelector(
      "#" + this.options.elementId
    );
